...
POST /search
  ...
  note: includes distanceMiles per item when zip provided; pageSize clamped to [1, 200]
...
POST /:id/confirm
  ...
  note: taxRate defaults to 8.25%; order must be PENDING

POST /:id/pay
  → Order
  note: order must be CONFIRMED (enforced — cannot skip invoice creation)

POST /:id/fulfill
  → Order
  note: order must be PAID

POST /:id/cancel
  body: { reason }
  → Order
  note: blocked for FULFILLED and REFUNDED orders

POST /bulk
  body: { orderIds: string[], updates: { tags?, dueDate?, status? } }
  → BulkOperation (201)
  note: PAID, REFUNDED statuses are blocked; undoDeadline = now + 10 min
...
POST /bulk/:id/undo
  ...
  note: fails with UNDO_EXPIRED if past undoDeadline
...
POST /ingest
  ...
  → { fileId, cases: ReconciliationCase[], summary: { matched, unmatched, variance, suspected_duplicate } }
  note: duplicate file (same SHA-256) is rejected with DUPLICATE_FILE
...
POST /cases/:id/dispose
  body: { disposition: "CONFIRM_MATCH"|"SPLIT"|"MERGE"|"WRITE_OFF", invoiceIds?, mergeWithCaseId?, reason? }
  → ReconciliationCase
  note: SPLIT requires invoiceIds (array, ≥ 2); MERGE requires mergeWithCaseId
...
PackageVersion   { version, composition: [{examItemId, required}], price, deposit, validityDays, validFrom }
Order            { id, patientId, patient, packageId, packageVersion, snapshot, status, tags, dueDate, invoiceId, tenantId, createdAt }
Invoice          { id, orderId, lines, subtotal, discount, taxRate, tax, total, currency, status, paidAt }
BulkOperation    { id, tenantId, patch, before, appliedAt, undoDeadline, undone }
ReconciliationCase { id, fileId, amount, counterparty, memo, date, status, disposition, matchedInvoiceId, score, reviewer }
